...
class CapsuleNet(nn.Module):  # CapsuleNet = DisenGCN
    def __init__(self, nfeat, hyperpm):
        ...
        self.pca2 = SparseInputLinear(nfeat, rep_dim)
    ...
    def forward(self, x, nb):
        ...
        x2 = fn.leaky_relu(self.pca2(x)).to(device)#logstd
        x_initial1 = x1
        x_initial2 = x2
        ...
        for i, conv in enumerate(self.conv_ls):
                if i <= self.nlayer-1:
                    continue
                x2 = self._dropout((conv(x2, nb, self.routit, param)))
                if i == 2*self.nlayer-2:
                    break
        x2 = conv(x2, nb, self.routit, param).to(device)
              
        return x_initial1, x_initial2, x1,x2
