...
parser.add_argument('--seed', type=int, default=23,
                        help='random seed')
parser.add_argument('--flayers', type=int, default=3,
...
parser.add_argument('--flow', type=str, default=True, help="whether use flow only for name")
...
def main():
    ...
    print(features)
    vgae_model = model.VGAEModel(dataset,features, args)
    ...
    save_dir = './results/{}/{}_{}_{}_{}_weight_{}_seed_{}/'.format(
        args.datname, args.encoder, args.decoder,args.flow,args.permute, str(args.weight),str(test_seeds))
    ...
    for epoch in range(args.epochs):
        ...
        mean, flow_mean,z,log_det, logits = vgae_model.forward()
        ...
        loss += kl_divergence
        loss -= 1/logits.size(0)*log_det.mean()
        ...
        log = ('Train Epoch: {} \t train_loss: {:.5f}, cross_entropy: {:.5f}, kl_divergence: {:.5f}, -log_det.mean: {:.5f}, '
                   'disentangle_loss: {:.5f}, train_acc: {:.5f}, val_roc: {:.5f}, val_ap: {:.5f}, time: {:.5f}'.format(
                epoch+1, loss.item(),
                norm*F.binary_cross_entropy(
            logits.view(-1), adj_label.to_dense().view(-1), weight=weight_tensor)
                       , kl_divergence.item(), -1/logits.size(0)*log_det.mean(), vgae_model.compute_disentangle_loss(args.weight).item(),
