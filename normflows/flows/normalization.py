...
#             z.size()[0]
#         )
        minz, min_indices  = torch.min(z, dim=0, keepdims=True)
       
        
        maxz, max_indices  = torch.max(z, dim=0, keepdims=True)
       
        
        z_ = 2 * (z - minz) /(maxz + self.eps) - 1
        log_det = torch.log(2 / torch.prod((maxz + self.eps))).repeat(
            z.size()[0]
        )
        return z_, log_det
