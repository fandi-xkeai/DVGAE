...
class Permute(Flow):#当mode="shuffle"时，num_channels应该等于隐变量维度，这样shuffle是对全体的，才不会遗漏数值，如38行
    """
    ...
    def __init__(self, num_channels, mode="shuffle"):
        ...
        """#特征排列方式，可以shuffle进行随机排列，也可以swap进行上下部分互换
        super().__init__()
        self.mode = mode
        self.num_channels = num_channels
        if self.mode == "shuffle":
            perm = torch.randperm(self.num_channels)#torch.randperm(n)：将0~n-1（包括0和n-1）随机打乱后获得的数字序列
            inv_perm = torch.empty_like(perm).scatter_(
                dim=0, index=perm, src=torch.arange(self.num_channels)
            )
            #这步操作得到的inv_perm和torch.randperm()操作类似，就是0-（self.num_channels-1）乱序的一维tensor
            #torch.empty_like()是创建一个与input形状一样的使用未初始化值填满的tensor，元素都是0
            #按序输出整数; torch.arange()默认从0开始，一直到输入的数-1，向量内部元素默认为整数；
            #scatter_讲解https://blog.csdn.net/Aaron_neil/article/details/130168355
    ...
    def inverse(self, z, context=None):
        if self.mode == "shuffle":
            ...
            raise NotImplementedError("The mode " + self.mode + " is not implemented.")
        log_det = torch.zeros(len(z), device=z.device)#置换矩阵的行列式为1或者-1，那么绝对值为0，因此log1=0
        return z, log_det
