...
class AffineConstFlow(Flow):
    ...
    def forward(self, z):
        z_ = z * torch.exp(self.s) + self.t#正态标准化
...
#                 print('scale_',scale_.size())
#                 print('shift',shift.size())
#                 print('z2',z2.size())
                z2 = z2 * torch.exp(scale_) + shift
                #print('scale_',scale_)
                log_det = torch.sum(scale_, dim=list(range(1, shift.dim())))
                #print('coupling_logdet',log_det)
...
class AffineCouplingBlock(Flow):
    ...
    def __init__(self, param_map, scale=True, scale_map="exp", split_mode="channel"):
        ...
        # Split layer
        self.flows += [Split(split_mode)]#Split:拆分；Split features into two sets；flows--reshape.py
        # Affine coupling layer
        self.flows += [AffineCoupling(param_map, scale, scale_map)]
        # Merge layer
        self.flows += [Merge(split_mode)]#Merge：融合层
